...
The Choi matrices use the unnormalized project convention `C_\mathcal{E}` with
input system `A` first and output system `B` second, so trace preservation is
`Tr_B(C_\mathcal{E}) = I_A`.
...
- `qpt_tools.py` - local helper module for Choi construction, linear inversion, CVXPY-based CPTP/MLE projection, true diamond-norm SDP diagnostics, plotting, and optional Qiskit Experiments submission.
...
For one-qubit gates, the linear inversion path uses the informationally complete inputs `|0>`, `|1>`, `|+>`, and `|+i>`.  The MLE step solves the nearest CPTP Choi projection with CVXPY:
...
When CVXPY is available, the MLE solver tries SCS first, then CLARABEL with
solver-specific options separated so the notebook is portable across conic
solver installations.

The diagnostics now report both the true half-diamond distance
`0.5 * ||\mathcal{E}_{actual}-\mathcal{E}_{ideal}||_\diamond`, computed by a
local SDP, and a separately labeled `diamond_distance_proxy` based on a Choi
nuclear norm. Only the SDP value should be called a diamond norm/distance in
the final report.
...
The notebook includes a commented code cell showing this pattern.  It is not executed by default, and no hardware submission happens during the reproducible offline run.
...
The optional Qiskit/IBM dependencies are only needed for real backend submission.  The tests and default notebook path exercise the reproducible offline implementation.  The versions in `requirements.txt` are pinned to the validated `qiskit_2025_1` environment used for this revision.

## Key Results in the Sample Output

The sample data report process fidelity, unclipped raw process fidelity, average gate fidelity, CP/TP status, Kraus weights, true half-diamond distance, a clearly labeled diamond-distance proxy, and a heuristic noise label for three gates.  The values are simulated, not hardware claims.  They are meant to make downstream validation deterministic while preserving the same Choi-based analysis path used for hardware data.
